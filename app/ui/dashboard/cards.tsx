...
export function Card({
  ...
  return (
    <div className={`rounded-xl bg-gray-50 ${theme.container} p-2 shadow-sm`}>
      <div className="flex p-4">
        {Icon ? <Icon className={`h-5 w-5 text-gray-700 ${theme.text}`} /> : null}
        <h3 className={`ml-2 text-sm font-medium ${theme.title}`}>{title}</h3>
      </div>
      <p className={`${lusitana.className} truncate rounded-xl
      bg-white ${theme.bg} ${theme.title} px-4 py-8 text-center text-2xl`}
      >
        {value}
      </p>
    </div>
